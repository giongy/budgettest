...
def compute_budget_distribution(year_amount, year_period, month_bids, overrides):
    ...
    if over_limit:
        ...
        if not missing_bids:
            for bid in month_bids:
                values[bid] = overrides.get(bid, 0.0)
            if expected_count and len(month_bids) < expected_count:
                total_display = annual_total
            else:
                total_display = sum_overrides
...
class BudgetApp(QWidget):
    def __init__(self):
        ...
        # Avoid single-click edits so the inline icon can be clicked without entering edit
    ...
    def apply_light_theme(self):
        self.setStyleSheet(
            """
            QWidget { background-color: white; color: black; font-size: 10px; }
            QPushButton { background-color: #f3f4f6; border: 1px solid #ccc; border-radius: 5px; padding: 4px 10px; font-size: 10px; }
            QPushButton:hover { background-color: #e2e6ea; }
            QHeaderView::section { background-color: #f2f2f2; color: #111; font-weight: bold; font-size: 10px; }
            QTreeView { alternate-background-color: #fafafa; gridline-color: #d0d0d0; font-size: 10px; }
            QTreeView::item { border: 0px; }
            QTreeView::item:selected { outline: none; border: 0px; }
    ...
    def refresh(self):
        ...
        def format_header(name: str) -> str:
            if len(name) == 7 and name[4] == "-" and name[:4].isdigit() and name[5:].isdigit():
                year_part = name[:4]
                month_num = int(name[5:])
                if 1 <= month_num <= 12:
                    return f"{ITALIAN_MONTHS[month_num]} {year_part}"
            return name
        if entries:
            year_id, year_name = entries[0]
            header_names.append(format_header(year_name))
            header_names.append("Period")
            header_ids.append(year_id)
            for bid, name in entries[1:]:
                header_names.append(format_header(name))
        ...
        def add_category(cid, depth=0):
            ...
            if depth == 0:
                cat_item.setBackground(QBrush(QColor("#E3F2FD")))
            ...
            # Light shading for entire Actual row on main categories
            if depth == 0:
                for idx, it in enumerate(act_row):
                    it.setBackground(QBrush(QColor("#E3F2FD")))

            # If no actual at all for this category, grey out Year, Period and TOTAL cells
            if not has_any_actual:
                grey = QBrush(QColor("#EEE"))
                if len(act_row) > 1 and act_row[1]:
                    act_row[1].setBackground(grey)
                if len(act_row) > 2 and act_row[2]:
                    act_row[2].setBackground(grey)
                if act_row:
                    act_row[-1].setBackground(grey)
            ...
            if depth == 0:
                for idx, it in enumerate(bud_row):
                    it.setBackground(QBrush(QColor("#E3F2FD")))
            ...
            if depth == 0:
                for idx, it in enumerate(diff_row):
                    it.setBackground(QBrush(QColor("#E3F2FD")))
            else:
                for idx, it in enumerate(diff_row):
                    if idx == 0 and it.text() == "Diff":
        ...
        for col in range(1, self.model.columnCount()):
            name = header_names[col] if col < len(header_names) else ""
            if name == "Period":
    ...
    def recalc_category(self, cid: int):
        ...
        try:
            ...
            # Keep background highlight on the annual (year) cell if edited
            year_item = target.child(budget_row_idx, 1)
            if year_item:
                if (year_bid, cid) in self.edits:
                    year_item.setBackground(QColor("#FFF59D"))
                elif depth == 0:
                    year_item.setBackground(QBrush(QColor("#E3F2FD")))
                else:
                    year_item.setBackground(QBrush())
            ...
            if year_item:
                if (year_bid, cid) in self.edits:
                    ...
                    year_item.setBackground(QBrush())import sys
from pathlib import Path
        ...
        try:
            ...
            for idx, bid in enumerate(month_bids, start=3):
                ...
                if item:
                    item.setText(f"{val:,.2f}" if val else "")
                                        if (bid, cid) in self.edits:
                        item.setBackground(QColor(\# Update total cell and color
            ...
            if tot_item:
                ...
                if over_limit:
                    ...
                    if depth == 0:
                        tot_item.setBackground(QBrush(QColor("#E3F2FD")))
                    else:
